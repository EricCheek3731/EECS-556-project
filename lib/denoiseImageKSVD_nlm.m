function [IOut,output] = denoiseImageKSVD_nlm(Image,sigma,K,varargin)
%==========================================================================
...
for jj = 1:30000:size(blocks,2)
    ...
    % beta
    beta = nlm_filter2D_patch(Coefs,NN1,NN2,16,20,sigma);
    % update alpha
    lambda = 2.5;
    rho = 1.7;
    alpha = 1;
    
    for i = 1:1000:size(beta,2)
        Block = blocks(:,jj:jumpSize);
        Coefs_new = lasso(Dictionary, Block(:,i) - Dictionary*beta(:,i), lambda/2, rho, alpha);
        Coefs(:,i) = Coefs_new + beta(:,i);
    end
