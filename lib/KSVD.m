...
else %preserveDCAtom in denoiseImageKSVD is 1
...
if (size(Data,2) < param.K)
    ...
    output.CoefMatrix = eye(size(Data,2));% added
    return;
...
elseif (strcmp(param.InitializationMethod,'GivenMatrix')) % InitializationMethod in denoiseImageKSVD is 'GivenMatrix'
...
if (param.preserveDCAtom)
    tmpMat = FixedDictionaryElement \ Dictionary; % \ sove x for Ax = B
...
for iterNum = 1:param.numIteration
    ...
    else % errorFlag in denoiseImageKSVD function is 1
    ...
    if (displayErrorWithTrueDictionary ) 
        % I_findDistanseBetweenDictionaries 
        [ratio(iterNum+1),ErrorBetweenDictionaries(iterNum+1)] = I_findDistanseBetweenDictionaries(param.TrueDictionary,Dictionary); % no TrueDictionary
...
Dictionary = [FixedDictionaryElement,Dictionary]; % Update dictionary
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
